...
export class TestGenerator {
  ...
  async generate(config: TestConfig): Promise<GeneratedTest & { content: string }> {
    ...
    // Step 2: Generate test code using AI
    const generatedCode = await this.generateTestCode(config, ragContexts);

    // Step 3: Post-process and format the code
    ...
    // Step 5: Write to file (if not dry run)
  ...
  private async retrieveContext(config: TestConfig): Promise<RAGContext[]> {
    ...
    return await this.ragService.getWorkflowContext(query);
  }
...
Return ONLY the TypeScript test code, no explanations.`;
  ...
  private calculateConfidence(contexts: RAGContext[], generatedCode: string): number {
    ...
    // Boost confidence based on context coverage
    ...
    // Reduce confidence for potential issues in generated code
    if (generatedCode.includes('text=')) confidence -= 0.1; // Brittle text selectors
    if (generatedCode.includes('waitForTimeout')) confidence -= 0.15; // Hard waits
    if (!generatedCode.includes('expect(')) confidence -= 0.2; // Missing assertions
