...
export class TestGenerator {
     ...
     */
    async optimizeFinalCodeWithOpenAI(code) {
    ...
    async generate(config) {
        ...
        const isLoginTest = /login|authentication|credentials|error message|invalid/i.test(config.ticket.summary + config.ticket.description + (config.ticket.acceptanceCriteria || []).join(' '));
        ...
        if (this.shouldPrependLogin(config)) {
            ...
            generatedCode = generatedCode.replace(/(test\.beforeEach\s*\(\s*async\s*\(\{\s*page\s*\}\s*\)\s*=>\s*{)/, `$1\n    ${loginSteps}\n`);
        }
    ...
    async generateTestCode(config, contexts) {
...
- Implement auto-retry with proper timeouts`;
    ...
    calculateConfidence(contexts, generatedCode) {
        let confidence = 0.5;
        ...
        if (workflowContexts > 0)
            confidence += 0.1;
        if (selectorContexts >= 5)
            confidence += 0.1;
        if (generatedCode.includes('text='))
            confidence -= 0.1;
        if (generatedCode.includes('waitForTimeout'))
            confidence -= 0.15;
        if (!generatedCode.includes('expect('))
            confidence -= 0.2;
        if (generatedCode.includes('getByTestId'))
            confidence += 0.1;
        if (generatedCode.includes('getByRole'))
            confidence += 0.05;
        if (generatedCode.includes('toBeVisible'))
            confidence += 0.05;
        if (generatedCode.includes('process.env'))
            confidence += 0.05;
        return Math.max(0, Math.min(1, confidence));
    }
    extractTestName(summary) {
        return summary
            .toLowerCase()
            .replace(/[^a-z0-9\s]/g, '')
            .replace(/\s+/g, ' ')
